...
class Pagination {
	...
	public function build($total_posts, $per_page, $current_page='') {
		...
		if ( $total_pages > 1 ) {
			...
			// Back link.
			if ( $page>1 ) $this->html .= '<a href="'.$this->url->show('page='.($page-1)).'"><i class="fa fa-arrow-left"></i></a>';
			
			for ( $i = ($page - $range); $i < ($page + $range + 1); $i++ ) {
				
				if ( ($i > 0) && ($i <= $total_pages) ) {
					
					if ( $i == $page ) $this->html .= '<a href="'.$this->url->show('page='.$i).'" class="active">'.$i.'</a>';
					else $this->html .= '<a href="'.$this->url->show('page='.$i).'">'.$i.'</a>';
			...
			// Forward link.
			if ($page != $total_pages) $this->html .= '<a href="'.$this->url->show('page='.($page+1)).'"><i class="fa fa-arrow-right"></i></a>';
	...
	public function html($container_class='') {
		if ( !empty($this->html) ) echo '<div class="pagination '.$container_class.'"><div class="pages">'.$this->html.'</div></div>';
	}
