...
if ( !empty( $_FILES ) ) {
	...
	if ( empty( $error->selected ) ) {
		...
		if ( !in_array( $user->info('avatar'), $d_avatars ) )
			@unlink( ABS . 'uploads/avatars/' . $user->info('avatar') );
		
		$avatar_file = trim( $images, ',' );
		
		$db->where( array( 'id' => $user->info('id') ) )->update( 'users', array( 'avatar' => $avatar_file ) );
...
<form action="/account?tab=avatar" method="POST" class="form submission" enctype="multipart/form-data">
    ...
    <div class="buttons-cont">
        <button type="submit" id="submit" class="bttn green large"><i class="fa fa-upload"></i> Upload Avatar</button>
    </div>
