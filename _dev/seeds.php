...
$pi = [
	...
	'seo_description'	=> 'MCPE\'s largest collection of Minecraft PE seeds. The most up to date list of MCPE seeds in Minecraft.',
	'seo_keywords'		=> 'minecraft pe seeds, flat seeds, minecraft pe, mcpe'
];

show_header('Minecraft PE Seeds', FALSE, $pi);
...
$cats = ['Caverns', 'Mountains', 'Flat', 'Diamonds', 'Lava', 'Overhangs', 'Waterfall'];
...
<div class="search<?php if ( !isset($_GET['category']) && !empty($_GET['search']) ) echo ' visible'; ?>">
    <form action="<?php echo $url->show('', 1); ?>" method="GET" class="form">
        <input type="text" name="search" id="search" placeholder="Search Seeds..." value="<?php $form->GET_value('search'); ?>" maxlength="150">
...
foreach( $posts as $i => $p ) {
	...
	$p['featured_html'] = ( $p['featured'] == 1 ) ? '<div class="featured"><i class="fa fa-star"></i> Featured Seed</div>' : NULL;
...
<div class="post">
    ...
    <div class="info">
        ...
        <div class="link"><a href="'.$p['url'].'" class="button dl silver"><i class="fa fa-external-link"></i> Show Seed Code</a></div>
    </div>
</div>';
