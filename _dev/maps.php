...
$pi = [
	...
	'seo_description'	=> 'Collection of the best Minecraft PE Maps and game worlds for download including adventure, survival, and parkour maps.',
	'seo_keywords'		=> 'minecraft pe maps, survival, parkour, adventure, minecraft pe, mcpe'
];

show_header('Minecraft PE Maps', FALSE, $pi);
...
<div class="search<?php if ( !isset($_GET['category']) && !empty($_GET['search']) ) echo ' visible'; ?>">
    <form action="<?php echo $url->show('', 1); ?>" method="GET" class="form">
        <input type="text" name="search" id="search" placeholder="Search Maps..." value="<?php $form->GET_value('search'); ?>" maxlength="150">
...
foreach( $posts as $i => $p ) {
	...
	$p['featured_html'] = ( $p['featured'] == 1 ) ? '<div class="featured"><i class="fa fa-star"></i> Featured Map</div>' : NULL;
