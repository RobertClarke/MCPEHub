...
if ( !empty( $profile['username'] ) && !$user->suspended( $profile['username'] ) && $user_db = $user->info( '', $profile['username'] ) ) {
	...
	$profile['avatar'] = './avatar/110x110/'.$profile['avatar'];
...
<div id="profile">
    ...
    <div class="head section clearfix">
        ...
        <div class="usr-info">
            ...
            <h5><?php echo $user->badges( $profile['username'] ); ?> Last active <?php echo since( strtotime( $profile['last_active'] ) ); ?></h5>
            <div class="actions">
...
<?php
	...
	foreach( $posts as $post ) { // START: Post foreach.
		...
		$post['thumb'] = './uploads/700x50/'.$post['type'].'s/'.$post['image'];
		$post['thumb_a'] = './avatar/32x32/'.$user->info('avatar', $post['author']);
		...
		$post['featured_html'] = ( $post['featured'] == 1 ) ? '<div class="featured"><i class="fa fa-star"></i> Featured Map</div>' : NULL;
...
<div class="post">
    <div class="img">
        '.$post['featured_html'].'
        <div class="overlay">
            <a href="'.$post['url_a'].'"><img src="'.$post['thumb_a'].'" class="avatar" alt="'.$post['auth'].'" width="32" height="32"></a>
    ...
    <div class="info">
        <ul>
            <li><i class="fa fa-thumbs-up"></i> <b>'.$post['likes'].'</b> likes</li>
            <li><i class="fa fa-eye"></i> <b>'.$post['views'].'</b> views</li>
            <li><i class="fa fa-comments"></i> <b>'.$post['comments'].'</b> comments</li>
        </ul>
...
<?php if ( isset( $pagination_html ) ) echo '<center>'.$pagination_html.'</center>'; ?>
	    </div>
