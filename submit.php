...
foreach( $type_url as $id => $p ) {
	$active = ( $id == $post_type ) ? 'active ' : '';
	$type_urls .= '<a href="'.$c_url->show('type='.$id, TRUE).'" class="'.$active.'bttn"><i class="fa fa-'.$p[1].'"></i> '.$p[0].'</a>';
}
...
<div id="page-title">
    <h2>Submit <?php echo ucwords($post_type); ?></h2>
    <ul class="tabs">
        <?php echo $type_urls; ?>
    </ul>
</div>
...
<form action="<?php echo $c_url->show('type='.$post_type); ?>" method="POST" class="form submission" enctype="multipart/form-data">
...
<?php } ?>
    ...
    <div class="buttons-cont">
        <button type="submit" id="submit" class="save"><i class="fa fa-upload"></i> Submit <?php echo ucwords($post_type); ?></button>
    </div>
